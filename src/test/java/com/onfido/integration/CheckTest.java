...
public class CheckTest extends TestBase {
  ...
  public void createCheckTest() throws Exception {
    Check check = createCheck(applicant, document, new CheckBuilder().reportNames(Arrays.asList(ReportName.DOCUMENT)));
  ...
  public void createSubResultCheckTest() throws Exception {
    Check check = createCheck(applicant, document, new CheckBuilder().reportNames(Arrays.asList(ReportName.DOCUMENT)).subResult("rejected"));
    Assertions.assertEquals(applicant.getId(), check.getApplicantId());
  }

  @Test
  public void createConsiderCheckTest() throws Exception {
    Check check = createCheck(applicant, document, new CheckBuilder()
  ...
  public void createDrivingLicenceCheckTest() throws Exception {
    Check check = createCheck(applicant, document,
      new CheckBuilder().reportNames(Arrays.asList(ReportName.DOCUMENT))
                        ...
                        .usDrivingLicence(new UsDrivingLicenceBuilder()
                                .idNumber("12345")
                                .state("GA")));
  ...
  public void listChecks() throws Exception {
    createCheck(applicant, document,
      new CheckBuilder().reportNames(Arrays.asList(ReportName.DOCUMENT)));
