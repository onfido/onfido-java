...
public class TestBase {
  ...
  protected SigningDocument uploadSigningDocument(Applicant applicant, String filename)
    ...
    return onfido.uploadSigningDocument(
        applicant.getId(), new FileTransfer(new File("media/" + filename))).execute();
  }
  ...
  protected Check createCheck(Applicant applicant, Document document, CheckBuilder checkBuilder)
    ...
    return onfido.createCheck(
        checkBuilder.applicantId(applicant.getId()).documentIds(Arrays.asList(document.getId()))).execute();
  }

  protected WorkflowRun createWorkflowRun(UUID workflowId, UUID applicantId) throws Exception {
    return onfido.createWorkflowRun(
        new WorkflowRunBuilder().workflowId(workflowId).applicantId(applicantId)).execute();
  }
  ...
  public void cleanUpApplicants() throws IOException, ApiException {
    ...
    for (Applicant applicant : onfido.listApplicants().page(1).perPage(100).includeDeleted(false).execute().getApplicants()) {
  ...
  public WatchlistMonitor createWatchlistMonitor(
    ...
    return onfido.createWatchlistMonitor(
        new WatchlistMonitorBuilder().applicantId(applicantId).reportName(reportName)).execute();
  }
